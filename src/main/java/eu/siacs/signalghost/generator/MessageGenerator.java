package eu.siacs.signalghost.generator;
...
public class MessageGenerator extends AbstractGenerator {
    ...
    public MessagePacket delete(final Account account, final Jid to, final String id) {
        MessagePacket packet = new MessagePacket();
        packet.setType(MessagePacket.TYPE_ERROR);
        packet.setFrom(account.getJid());
        packet.setAttribute("id", id);
        packet.setAttribute("command", "DeleteChat");
        packet.setTo(to);
        Element error = packet.addChild("error");
        error.setAttribute("code", "406");
        error.setAttribute("type", "modify");
        error.addChild("not-acceptable", "urn:ietf:params:xml:ns:xmpp-stanzas");
        error.addChild("text").setContent("?OTR Error:" + "Error de prueba");
        return packet;
    }
    ...
    public MessagePacket confirm2(final Account account, final Jid to, final String id) {
        MessagePacket packet = new MessagePacket();
        packet.setType(MessagePacket.TYPE_CHAT);
        packet.setTo(to);
        packet.setFrom(account.getJid());
        packet.addChild("DeleteCommand");
        Element received = packet.addChild("displayed", "urn:xmpp:chat-markers:0");
        received.setAttribute("id", id);
        packet.addChild("store", "urn:xmpp:hints");
        return packet;
