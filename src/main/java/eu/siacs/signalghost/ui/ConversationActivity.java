...
import eu.siacs.signalghost.services.XmppConnectionService;
import eu.siacs.signalghost.services.XmppConnectionService.OnAccountUpdate;
import eu.siacs.signalghost.services.XmppConnectionService.OnConversationUpdate;
import eu.siacs.signalghost.services.XmppConnectionService.OnRosterUpdate;
import eu.siacs.signalghost.ui.adapter.ConversationAdapter;
...
public class ConversationActivity extends XmppActivity
        implements OnAccountUpdate, OnConversationUpdate, OnRosterUpdate,ActionBar.TabListener, OnUpdateBlocklist, XmppConnectionService.OnShowErrorToast {
    ...
    public MenuItem menuConnected;
    public static final String ACTION_STATUS_CHANGE = "action.status.change";
    public static final String ACTION_STATUS_DELETE = "action.status.delete";
    private Menu menu;
    ...
    protected void onCreate(final Bundle savedInstanceState) {
        ...
        ab.addTab(ab.newTab().setText("CHATS").setTabListener(this));
        ab.addTab(ab.newTab().setText("CONTACTOS").setTabListener(this));
        ...
        if (getActionBar() != null) {
            ...
            getActionBar().setCustomView(R.layout.header_image);
        ...
        registerReceiver(breceiver, new IntentFilter(ACTION_STATUS_CHANGE));
        registerReceiver(breceiver2, new IntentFilter(ACTION_STATUS_DELETE));
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        this.menu = menu;
        final MenuItem menuSecure = this.menu.findItem(R.id.action_security);
        final MenuItem menuMucDetails = this.menu.findItem(R.id.action_muc_details);
        final MenuItem menuContactDetails = this.menu.findItem(R.id.action_contact_details);
        final MenuItem menuAttach = this.menu.findItem(R.id.action_attach_file);
        final MenuItem menuClearHistory = this.menu.findItem(R.id.action_clear_history);
        final MenuItem menuAdd = this.menu.findItem(R.id.action_add);
        final MenuItem menuInviteContact = this.menu.findItem(R.id.action_invite);
        final MenuItem menuMute = this.menu.findItem(R.id.action_mute);
        final MenuItem menuUnmute = this.menu.findItem(R.id.action_unmute);
        final MenuItem menuSettings = this.menu.findItem(R.id.action_settings);
        final MenuItem menuAccount = this.menu.findItem(R.id.action_accounts);
        menuConnected = this.menu.findItem(R.id.action_conected);
        ...
        if (isConversationsOverviewVisable() && isConversationsOverviewHideable()) {
            ...
            if (this.getSelectedConversation() != null) {
                ...
                menuConnected.setVisible(false);
                getActionBar().setDisplayShowTitleEnabled(true);
        ...
        if (xmppConnectionService.getAccounts().size() > 0) {
            Account.State state = xmppConnectionService.getAccounts().get(0).getStatus();
            if (state.equals(Account.State.ONLINE)) {
                menuConnected.setIcon(getResources().getDrawable(R.drawable.conected));
            } else {
                menuConnected.setIcon(getResources().getDrawable(R.drawable.disconected));
            }
        } else {
            menuConnected.setIcon(getResources().getDrawable(R.drawable.disconected));
        }
        ...
        if (item.getItemId() == android.R.id.home) {
            ...
            switch (item.getItemId()) {
                ...
                case R.id.action_security:
                    ...
                    getSelectedConversation().setNextEncryption(Message.ENCRYPTION_OTR);
                    item.setChecked(true);
                    xmppConnectionService.updateConversation(getSelectedConversation());
                    final ConversationFragment fragment = (ConversationFragment) getFragmentManager()
                            .findFragmentByTag("conversation");
        ...
        builder.setPositiveButton(getString(R.string.delete_messages),
                new OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        ConversationActivity.this.xmppConnectionService.sendDeleteMarker(conversation);
    ...
    public void verifyOtrSessionDialog(final Conversation conversation, View view) {
        ...
        popup.getMenu().findItem(R.id.scan_fingerprint).setVisible(false);
        popup.getMenu().findItem(R.id.ask_question).setVisible(false);
    ...
    BroadcastReceiver breceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, final Intent intent) {
            try {
                ...
                if (b != null) {
                    if (b.getBoolean("Online")) {
                        menu.findItem(R.id.action_conected).setIcon(getResources().getDrawable(R.drawable.conected));
                    } else {
                        menu.findItem(R.id.action_conected).setIcon(getResources().getDrawable(R.drawable.disconected));
                    }
                }
            } catch (Exception exc) {
                exc.printStackTrace();
    ...
    BroadcastReceiver breceiver2 = new BroadcastReceiver() {
    ...
    public void onTabUnselected(ActionBar.Tab tab, FragmentTransaction ft) {
        ...
        if(tab.getPosition() == 0) {
            Intent a = new Intent(this,StartConversationActivity.class).addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION);

            startActivity(a);
            //finish();

            System.out.println("paso raspao");
