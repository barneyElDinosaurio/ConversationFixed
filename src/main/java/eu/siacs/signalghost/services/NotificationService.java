...
import eu.siacs.signalghost.ui.ManageAccountActivity;
import eu.siacs.signalghost.ui.TimePreference;
import eu.siacs.signalghost.utils.GeoHelper;
...
public class NotificationService extends Activity {
    ...
    public void notificationWithLayout(String packageName){

        RemoteViews contentView = new RemoteViews(packageName,R.layout.custom_notification);
        contentView.setImageViewResource(R.id.image,R.drawable.ic_action_download);
        contentView.setTextViewText(R.id.title,"custon notification");
        contentView.setTextViewText(R.id.text,"this is a custon notification");
...
//        }
    ...
    private Builder buildSingleConversations(final ArrayList<Message> messages) {
        ...
        if (messages.size() >= 1) {
            ...
            if (Config.HIDE_MESSAGE_TEXT_IN_NOTIFICATION) { //TODO modificación de texto
                int count = messages.size();
            ...
            today.setToNow();
            //String cDate = String.valueOf();



            SimpleDateFormat df = new SimpleDateFormat("dd-MM-yyyy hh:mm:ss",new Locale("ES_es","CO"));
            int weekDay = today.weekDay;
            int month = today.month;
            //String weekDay = String.valueOf(today.);

            String fmont = today.format("%B");
            String fTime = today.format("%R %p");
            String fDay = String.valueOf(today.monthDay);
            System.out.println(month);

            mBuilder.setContentTitle(elegirDia(weekDay)+" "+fDay+" "+"de"+" "+fmont+ "   "+fTime);
            mBuilder.setSmallIcon(R.drawable.ic_notification);
            mBuilder.setContentText("Toca para desactivar alerta del sistema");
    ...
    public String elegirDia(int dia){

        String diaElegido = null;
